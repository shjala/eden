...
const (
	// DefaultSSHPass is a default ssh password for the VM running on the EVE node
	DefaultSSHPass = "passw0rd"
	// DefaultSSHUser is a default ssh user for the VM running on the EVE node
	DefaultSSHUser = "ubuntu"
	// DefaultCloudConfig is a default cloud-init configuration for the VM wich just
	// enables ssh password authentication and sets the password to "passw0rd".
	DefaultCloudConfig = "#cloud-config\npassword: " + DefaultSSHPass + "\nchpasswd: { expire: False }\nssh_pwauth: True\n"
)
...
// CreateEveNode creates a new RemoteNode struct
...
// WaitForAppRunningState waits for an app to start an become running on the EVE node
func (node *EveNode) WaitForAppRunningState(appName string, timeoutSeconds uint) error {
	start := time.Now()
	for {
		state, err := node.GetState(appName)
		if err != nil {
			return err
		}
...
func (node *EveNode) WaitForSSH(appName string, timeoutSeconds uint) error {
	start := time.Now()
	for {
		_, err := node.SSHExec(appName, "echo")
		if err == nil {
			return nil
		}
...
func (node *EveNode) StopAndRemoveApp(appName string) error {
...
// AddApp adds an app to the EVE node list of apps
func (node *EveNode) AddApp(appName, SSHUser, SSHPass, SSHPort string) {
	node.apps = append(node.apps, appInstanceConfig{name: appName, sshUser: SSHUser, sshPass: SSHPass, sshPort: SSHPort})
}

// GetState gets the state of an app running on the EVE node
func (node *EveNode) GetState(appName string) (string, error) {
...
// DeployVm deploys a VM on the EVE node
func (node *EveNode) DeployVm(imageURL string, pc openevec.PodConfig) error {
	return node.controller.PodDeploy(imageURL, pc, node.cfg)
}

// IsTpmEnabled checks if EVE node is running with (SW)TPM enabled
func (node *EveNode) IsTpmEnabled() bool {
	return node.cfg.Eve.TPM
}

func (node *EveNode) setEveIP() error {
...
// GetDefaultVmConfig returns the default configuration for a VM
func GetDefaultVmConfig(name, cloudConfig string, portPub []string) openevec.PodConfig {
...
// for running tests
func InitilizeTest(m *testing.M, projectName string) (*EveNode, error) {
